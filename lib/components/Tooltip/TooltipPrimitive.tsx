...
import { Slottable, useControllableState, useId } from '../..';
...
interface TooltipProviderProps {
  ...
  /**
   * When `true`, trying to hover the content will result in
   * the tooltip closing as the pointer leaves the trigger.
   * @defaultValue false
   */
  disableHoverableContent?: boolean;
}
...
const TooltipProvider: React.FC<TooltipProviderProps> = ({
  ...
  children,
}: ScopedProps<TooltipProviderProps>) => {
...
interface TooltipProps {
  ...
  delayDuration?: number;
  /**
   * When `true`, trying to hover the content will result
...
>(
  (
    { __scopeTooltip, ...triggerProps }: ScopedProps<TooltipTriggerProps>,
    forwardedRef,
  ) => {
    ...
    return (
      <PopperPrimitive.Anchor asChild {...popperScope}>
        <Primitive.button
          ...
          onBlur={composeEventHandlers(triggerProps.onBlur, context.onClose)}
          onClick={composeEventHandlers(triggerProps.onClick, context.onClose)}
        />
...
const TooltipPortal: React.FC<TooltipPortalProps> = ({
  ...
  container,
  children,
}: ScopedProps<TooltipPortalProps>) => {
...
>;
type PopperContentProps = Jelly.ComponentPropsWithoutRef<
  typeof PopperPrimitive.Content
>;
  ...
  (
    {
      ...
      ...contentProps
    }: ScopedProps<TooltipContentImplProps>,
    forwardedRef,
  ) => {
...
type PopperArrowProps = Jelly.ComponentPropsWithoutRef<
...
const TooltipArrow = React.forwardRef<TooltipArrowElement, TooltipArrowProps>(
  (
    { __scopeTooltip, ...arrowProps }: ScopedProps<TooltipArrowProps>,
    forwardedRef,
  ) => {
    const popperScope = usePopperScope(__scopeTooltip);
...
const getHull = <P extends Point>(points: Readonly<P[]>): P[] => {
...
const getHullPresorted = <P extends Point>(points: Readonly<P[]>): P[] => {
